...
class _ContactPageState extends State<ContactPage> {
  ...
  void initState() {
    ...
    if (widget.contact != null) {
      _nameController.text =
          widget.contact!.fields[AppKeys.name]?.toString() ?? "";
    }
    ...
    for (String key in allPossibleKeys) {
      ...
      List<String> remindBefore = key == AppKeys.birthday
          ? ["today"]
          : ["today"];
      ...
      if (val is Map) {
        ...
        remindYearly =
            val['remindYearly'] as bool? ?? (key == AppKeys.birthday);
  ...
  Future<void> _startTutorialIfNeeded() async {
    ...
    if (tutorialProvider == null || tutorialProvider.isContactTutorialShown) {
      return;
    }
    ...
    try {
      ...
      // Skip if the target is not mounted yet.
    }
    await tutorialProvider.markContactTutorialAsShown();
  }
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => IntelligentInputSheet(
          ...
          onToggleListening: () async {
            if (!isListening) {
              try {
                final speechService = Provider.of<SpeechService>(
                  context,
                  listen: false,
                );
                ...
                if (available) {
                  ...
                  String localeId = Localizations.localeOf(
                    currentContext,
                  ).toString();
              ...
              DropdownButton<FieldType>(
                ...
                items: [
                  DropdownMenuItem(
                    ...
                    child: Text(l10n.textType),
                  ),
                  DropdownMenuItem(
                    value: FieldType.number,
                    child: Text(l10n.numberType),
                  ),
                  DropdownMenuItem(
                    value: FieldType.date,
                    child: Text(l10n.dateType),
                  ),
                  DropdownMenuItem(
  ...
  Future<void> _confirmDeleteField(int index) async {
    ...
    final fieldName = key.isEmpty
        ? l10n.newField
        : AppKeys.getLocalizedLabel(key, l10n);
  ...
  Future<void> _confirmDeleteContact() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          l10n.deleteContactConfirmation(widget.contact?.name ?? l10n.noName),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
  ...
  void _renameField(int index) {
    ...
    final renameController = TextEditingController(
      text: _fields[index].keyController.text,
    );
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  Future<void> _saveContact() async {
    ...
    final nameError = Validators.validateContactName(
      name,
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                counterText: "",
                hintText: l10n.nameWithoutCommas,
              ),
            ),
  ...
  void _showManageGroupsBottomSheet(DynamicField field) {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => GroupManagerSheet(
        ...
        onEditGroup: (group, setModalState) =>
            _showGroupEditDialog(group, field, setModalState),
        ...
        actions: [
          if (widget.contact != null)
            IconButton(
              ...
              onPressed: _isSaving ? null : _confirmDeleteContact,
            ),
          if (_isSaving)
            const Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                ...
                children: _fields.map((field) {
                  if (field.valueController.text.trim().isEmpty &&
                      !_showEmptyFields) {
            ...
            Center(
              child: TextButton(
                onPressed: () =>
                    setState(() => _showEmptyFields = !_showEmptyFields),
                child: Text(
                  _showEmptyFields
                      ? l10n.hideEmptyFields
                      : l10n.showEmptyFields,
