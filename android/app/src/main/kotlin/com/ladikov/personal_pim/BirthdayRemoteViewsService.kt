...
class BirthdayRemoteViewsService : RemoteViewsService() {
    override fun onGetViewFactory(intent: Intent): RemoteViewsFactory {
        return BirthdayRemoteViewsFactory(this.applicationContext)
    }
}

data class BirthdayEntry(val id: String, val name: String, val date: String, val age: String)

class BirthdayRemoteViewsFactory(private val context: Context) : RemoteViewsService.RemoteViewsFactory {
    private var birthdays: List<BirthdayEntry> = listOf()

    override fun onCreate() {}

    override fun onDataSetChanged() {
        val prefs = context.getSharedPreferences("HomeWidgetPreferences", Context.MODE_PRIVATE)
        val birthdaysJson = prefs.getString("birthdays_data", "[]") ?: "[]"
        val jsonArray = JSONArray(birthdaysJson)
    ...
    override fun getCount(): Int = birthdays.size

    override fun getViewAt(position: Int): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.birthday_item)
        if (position >= birthdays.size) return views
        
        val entry = birthdays[position]
